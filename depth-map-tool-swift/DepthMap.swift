...
class DepthMap {
    static func createAVDepthData(ciContext: CIContext = CIContext(), grayscaleImage: CIImage) throws -> AVDepthData? {
        
        guard let pixelBuffer = createPixelBuffer(from: grayscaleImage, ciContext: ciContext) else {
            return nil
        }
        
        guard let depthPixelBuffer = try convertToDepthPixelBuffer(from: pixelBuffer) else {
            return nil
        }
    ...
    private static func convertToDepthPixelBuffer(from pixelBuffer: CVPixelBuffer) throws -> CVPixelBuffer? {
        let converter = try GrayscaleToDepthConverter()
